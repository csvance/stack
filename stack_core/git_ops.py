...
def cherry_pick(cwd: Path | str, sha: str, *, allow_empty: bool = True) -> None:
    """Cherry-pick ``sha`` onto HEAD in ``cwd``. Raises :class:`GitError` on conflict."""
    args = ["cherry-pick"]
    if allow_empty:
...
def cherry_pick_abort(cwd: Path | str) -> None:
    """Abort an in-progress cherry-pick. Silent if none is in progress."""
    _run(["git", "cherry-pick", "--abort"], cwd, check=False)


def conflicting_paths(cwd: Path | str) -> list[str]:
    """Files currently in conflict (``U`` status in the index)."""
    result = _run(
        ["git", "diff", "--name-only", "--diff-filter=U"], cwd, check=False
    )
    if result.returncode != 0:
        return []
...
def push_branch(
    ...
    *,
    ...
    """Push ``branch`` to ``remote``, defaulting to ``--force-with-lease``."""
    args = ["push"]
    if force_with_lease:
...
def soft_reset(cwd: Path | str, rev: str) -> None:
    _git(["reset", "--soft", rev], cwd)


def amend_no_edit(cwd: Path | str) -> None:
    _git(["commit", "--amend", "--no-edit"], cwd)


def checkout(cwd: Path | str, ref: str, *, create: bool = False, force: bool = False) -> None:
    args = ["checkout"]
    if force:
        args.append("-f")
    if create:
...
def create_branch(cwd: Path | str, name: str, *, at: str | None = None) -> None:
    args = ["branch", name]
    if at is not None:
        args.append(at)
    _git(args, cwd)
...
def delete_branch(cwd: Path | str, name: str, *, force: bool = False) -> None:
    args = ["branch", "-D" if force else "-d", name]
    _git(args, cwd)


def count_commits_between(cwd: Path | str, base: str, head: str) -> int:
    out = _git(["rev-list", "--count", f"{base}..{head}"], cwd)
    return int(out)
